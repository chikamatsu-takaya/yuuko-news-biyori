...
- [ ] **ニュース閲覧への遷移整理**：サイドバー「ニュースを見る」／ホーム「すべてを見る」押下で、いきなり記事詳細（ニュース閲覧画面）へ遷移している（`app/page.tsx` は news=NewsReaderScreen 直行・一覧を挟まない）。
  ...
  - Done when:
    ...
    - サイドバーの「ニュースを見る」を押したとき、記事詳細へ直接遷移しない
    - ホーム画面の既存ニュース一覧が表示される
    - ホーム画面の「すべて見る」がニュース一覧表示へ接続されている
    - 一覧のカードを選択すると、選択した記事IDを使用して記事詳細を開く
    - 記事詳細の「戻る」で、遷移元のニュース一覧へ戻る
    - 記事ID未指定時に暗黙の既定記事を開く導線を、通常操作では使用しない
    - UIテストまたは手動確認で、一覧から記事詳細までの遷移を確認できる
  - Review points:
    - 独立した新規ニュース一覧画面を重複して作成していない
    - 既存のホーム画面のニュース一覧を再利用している
    - 選択した記事IDが記事詳細へ正しく渡される
    - 記事詳細からの戻り先が遷移元と一致する
    - ニュース履歴画面の既存導線を壊していない
  - Notes:
    - 現在、ホーム画面とニュース履歴画面には実データ一覧が存在する
    - 現在のサイドバー「ニュースを見る」は記事詳細へ直接遷移する
    - ホーム画面の「すべて見る」は未配線
    - MVPでは既存のホーム画面のニュース一覧を入口として使用する
    - 独立したニュース一覧画面の新設は対象外
  - 想定：**一覧を経由してから**個別記事の詳細へ、という自然な遷移にする。
  - 決定：サイドバー「ニュースを見る」とホーム「すべて見る」の遷移先には、ホーム画面の既存ニュース一覧を使用する。独立したニュース一覧画面は新設しない。
