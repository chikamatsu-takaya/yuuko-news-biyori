...
- [ ] 月次ZIPとindexの整合確認後に元Markdownを退避付きで削除する
  ...
  - Status: Review
  - Owner: @codex
  - Branch: `codex/archive-markdown-retirement`
  - Issue/PR: #82
  - Done when:
    - v2記事カタログと月次ZIPの全記事整合性を確認した月だけを削除対象にできる
    - 削除前退避とロールバック経路があり、途中失敗・起動中断で記事を失わない
    - 復元済み・お気に入り・カタログ不完全・アーカイブ後に変更された記事を削除しない
    - 元Markdown削除後も履歴表示・取得済み判定・単記事復元が動作する
  - Notes:
    - 索引・履歴統合はPR #79、安全な単記事復元はPR #81でマージ済み
    - このPRでは画面ボタン配線・日次自動実行を行わない
...
- [ ] アーカイブ処理を日次で冪等に自動実行する
  ...
  - Done when:
    ...
    - 再起動・同日再実行・処理対象なしの冪等性をテストしている
  - Notes:
    - 退避付きMarkdown削除のマージ後に着手する
...
## 2. 現在地サマリー
- `develop` は **PR #81まで反映済み**。失敗時UI統一はDictionary/SettingsへのToast適用まで完了し、他画面への横展開が残件。
...
- **アーカイブ退避方針を確定(#57)し、増分1(#68)・記事索引/履歴統合(#79)・安全な単記事復元(#81)まで完了**: 元Markdownの退避付き削除はPR #82でレビュー中。日次自動化は後続。
...
- **次の優先順（非フロント）**: アーカイブ増分2（元Markdown削除＋ZIP再閲覧＋自動化）→ バックグラウンド常駐方式の決定・実装 → Playwright E2Eの必須化要否判断。
...
- [ ] 失敗時UI（トースト / 再試行 / フォールバック）を統一
  ...
  - Notes:
    ...
    - MainScreen / NewsReaderScreen / NewsHistoryScreen への横展開は別PRで対応予定
...
- [ ] 月次ZIPアーカイブ増分2（安全な復元・元Markdown削除・自動実行）
  ...
  - Done when:
    ...
    - ZIPとindexの整合確認後にだけ元Markdownを削除する（PR #82レビュー中）
    - ZIP内記事を安全に再閲覧できる ✅（PR #81）
    - 自動実行のタイミングと再実行時の冪等性がテストされている
  - Notes:
    - 実装順: 記事単位index・履歴統合（PR #79完了）→ 復元command（PR #81完了）→ 退避付き削除（PR #82レビュー中）→ 日次自動化
...
- [ ] バックグラウンド常駐方式を決定し、待機状態を実装する
  ...
  - Done when:
    ...
    - メイン画面を閉じても必要最小限の通知・日付確認だけが動く
    - 待機時とメイン画面表示時のCPU・メモリ差を再測定している
  - Notes:
    - 初回性能測定で、現状は単一の可視メインウィンドウのみと確認。要件定義書 §8.2 / §14の「通常時バックグラウンド待機」を満たすための後続
