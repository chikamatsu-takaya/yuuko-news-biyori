...
- [ ] アーカイブ処理を日次で冪等に自動実行する
  ...
  - Done when:
    ...
    - 再起動・同日再実行・処理対象なし・cleanup pendingの冪等性をテストしている
  - Notes:
    - 記事索引/履歴統合 #79、単記事復元 #81、退避付き削除 #82はマージ済み
    - 既存の30分低頻度チェックを共有し、専用スレッドは追加しない
...
## 2. 現在地サマリー
- `develop` は **PR #82まで反映済み**。失敗時UI統一はDictionary/SettingsへのToast適用まで完了し、他画面への横展開が残件。
...
- **アーカイブ退避方針を確定(#57)し、増分1(#68)・記事索引/履歴統合(#79)・安全な単記事復元(#81)・退避付き削除(#82)まで完了**: 日次自動化を実装中。
