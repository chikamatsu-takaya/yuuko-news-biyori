...
- [ ] ニュース履歴画面を実データへ接続する
  ...
  - Notes:
    - 保存済み記事の履歴API、画面表示、フィルタ、記事再閲覧導線まで実装済み。PR作成前のローカル検証段階
...
- [ ] アーカイブ退避の起点・粒度を決める
  ...
  - Issue/PR: 未定
  - Done when:
    - データ設計書 §14 の「1か月→月次ZIP」案と、記憶にある「4〜7日」案のどちらを採用するか決まっている
    - 採用方針がデータ設計書とこのチェックリストに反映されている
  - Notes:
    - お気に入り記事の扱い、容量、再表示体験への影響を一緒に決める
...
- **次の優先順**: ニュース履歴画面の実データ接続（実装済み・レビュー待ち） → 辞書メモ/削除UI → ゆうこ通知プレビュー → 設定画面未実装操作 → アーカイブ退避方針決定 → Playwright CI任意チェック → フロントUX整備(#46起票)。
...
- [ ] （B-4後続・決定）アーカイブ退避の起点・粒度：データ設計書 §14 は「1か月→月次ZIP（お気に入り除外）」。記憶の「4〜7日」案と要reconcile（4-7日採用時は §14 改訂）
  ...
  - Issue/PR: 未定
  - Done when:
    - 退避起点と粒度が決まり、データ設計書と実装タスクに反映されている
    - お気に入り・再表示・容量上限の扱いが明確になっている
...
- [ ] ニュース履歴画面を実データへ接続する
  ...
  - Notes:
    ...
    - 保存済み記事一覧APIと `NewsHistoryScreen` 接続まで実装済み。PR作成前のローカル検証段階
- [ ] ゆうこ辞書のメモ編集・削除・★操作をUIへ配線する
  - Priority: P1.5
  - Status: Next
  - Owner: 未定
...
- [ ] 設定画面の未実装操作を整理し、実装または明示的に無効化する
  ...
  - Done when:
    ...
    - 実装する操作はRust側commandへ寄せ、React側にファイル操作や外部通信を持たせていない
    - 後回しにする操作はユーザーに誤解されない表示になっている
  - Notes:
    - 根拠: 画面詳細設計書 SCR-003。AI接続テスト/リセットは設計書由来、キャッシュ削除/辞書エクスポート/アーカイブ管理は現UIの未実装ボタン由来
