...
## 2. 現在地サマリー
- `develop` は **PR #59 まで反映済み**（設定アクション方針整理＋リセット実装まで）。open PRは本PR（候補3）。
...
- **ゆうこ通知バックエンド強化は実装（本PR・候補3）**: クールタイム(60/120/180分)・日次上限(settings.notification.maxPerDay)・紹介済みFIFO・`request_yuuko_notification`（未紹介/未読/スコア順で選定＋ゲート）・dismiss/handle/ignoreのTSラッパー。報酬pendingは誤消ししない。画面接続は#46。
- **設計書突き合わせ結果**: 辞書メモ/削除UI、設定画面の未実装操作(#59完了)、ゆうこ通知(本PRでバックエンド完了)、正式identifier、権限/ログ/性能点検を追加追跡。
- **次の優先順**: 辞書メモ/削除UI配線 → フロントUX整備(#46。ゆうこ通知の画面接続・ホーム導線・用語選択) → 正式identifier決定 → リリース前セキュリティ/性能点検 → Playwright CI任意チェック。
...
- [ ] ゆうこ通知・軽量プレビューの操作結果をフロントへ接続する
  ...
  - Status: Doing
  - Owner: @claude
  - Branch: `feature/yuuko-notification-state`
  - Issue/PR: 本PR（候補3バックエンド）／ 残り画面接続は #46
  - Done when:
