...
MVPスコープ定義書 §4.1（基本デモシナリオ）の手順1〜10を基準にする（MVP必須の通しデモはニュース取得〜辞書保存まで）。友情ポイント・ランク進捗は追加機能のため、下の「追加機能の任意確認（A1〜A2）」へ分ける。各段階の詳細は失敗時の入口へ委譲する。
...
MVP必須の通しデモ確認は手順1〜10（辞書保存まで）。加えて MVP完了条件（MVPスコープ定義書 §14.1）では、保存した用語をゆうこ辞書画面で確認できること・再起動後も保持されることまでを必須とする。

### 追加機能の任意確認

次は実装済みだが、MVP必須の通し確認・完了条件には含めない。

| ID | 確認項目 | 期待結果 | 確認方法 | 関連入口 |
|---|---|---|---|---|
| A1 | 友情ポイント増加 | ニュース閲覧・用語解説でポイントが増える | 手動＋Rustテスト | 友情ランク |
| A2 | ランク進捗確認 | 簡易的なランク進捗が確認できる | 手動＋Rustテスト | 友情ランク |
